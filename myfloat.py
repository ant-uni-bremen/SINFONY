...
def compute_single_bitprob(floatx, p_s):
    ...
    p_b1 = np.sum(p_s[:, np.newaxis] * mop.int2bin(floatx.b_poss, floatx.N_bits), axis = 0)
    # Output probability for 0 and 1
    p_ba = np.concatenate(((1 - p_b1)[:, np.newaxis], p_b1[:, np.newaxis]), axis = -1)
    return p_ba
...
class float_toolbox():
    ...
    def __init__(self, float_name, mode = 0):
        self.float_name = float_name    # Floating point type, e.g., float16, float32, float64
    ...
    def comp_bitint2float(self, b):
        ...
        fl = (b2[:, 0], mop.bin2int(b2[:, 1:self.N_exp + 1], dtype = self.intx), mop.bin2int(b2[:, self.N_exp + 1:], dtype = self.intx))
        # fl[1] != 0 means implicit bit if exponent != 0
        x = (-1) ** fl[0] * (fl[2] * 2 ** - self.N_sig + self.floatx(1 * (fl[1] != 0))) * self.floatx(2) ** (fl[1].astype(np.int64) - self.expbias + (fl[1] == 0))
        # fl[1] == 2 ** self.N_exp - 1 (fl[1] == 31 for float16) means NaN, inf/-inf with fl[2] == 0
        x[fl[1] == 2 ** self.N_exp - 1] = np.NaN
        x[np.logical_and((fl[1] == 2 ** self.N_exp - 1), (fl[2] == 0))] = np.array([np.inf, -np.inf])
        return x
    ...
    def float2bitint(self, x):
        ...
        if self.mode == 0 and (self.float_name == 'float16' or self.float_name == 'float32' or self.float_name == 'float64'):
            ...
            indx[xisfin] = np.nanargmin(np.abs(x[xisfin, np.newaxis] - self.x_poss[np.newaxis, ...]), axis = -1)
            # No difference between +0/-0 -> +0:
            # Catch 0 and differentiate between +0/-0
            indx[xisfin][indx[xisfin] == 0] = np.signbit(x[xisfin][indx[xisfin] == 0]) * 2 ** (self.N_bits - 1)
            if x[xisfin].size != x.size:
                # Inf/-Inf exact
                indx[x == np.inf] = np.argmax(x[x == np.inf, np.newaxis] == self.x_poss[np.newaxis, ...], axis = -1)
                indx[x == -np.inf] = np.argmax(x[x == -np.inf, np.newaxis] == self.x_poss[np.newaxis, ...], axis = -1)
                # Difficult to discriminate NaNs/first one is chosen
                indx[np.isnan(x)] = np.argmax(np.logical_and(np.isnan(x)[np.isnan(x)][..., np.newaxis], np.isnan(self.x_poss)[np.newaxis, ...]), axis = -1)
    ...
    def float_errorcorrection(self, b, p_b):
        ...
        b_zneg = np.concatenate([np.ones(1, dtype = b.dtype), np.zeros(self.N_bits - 1, dtype = b.dtype)])[np.newaxis, ...]
        ...
        if iszneg.any():
            ...
            arg_p_b_min = np.argmin(np.concatenate([1 - p_b[iszneg, 0][:, np.newaxis], p_b[iszneg, 1:]], axis = -1), axis = -1)
