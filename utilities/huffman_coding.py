...
class huffman_coder():
    ...
    def huffman_tree_gen(self, symbol_with_probs):
        ...
        while len(nodes) > 1:
            ...
            newNode = self.Node(left.prob + right.prob,
...
if __name__ == '__main__':
    ...
    import semantic_communication.semantic_floating_point as semfl

    # Test Simulation
    sim = 1                                 # Test simulation with communication channel: 1
    # 0: Gaussian, 1: Uniform (continuous), 2: Data based (gradients), 3: Data based (gradients+models), 4: Semcom, 5: Uniform (per class)
    prior = 0
    Nb = 1000 			                    # 100, 1000, 10000/100000
    # Floating point object: Float4/8/16
    floatx = mfl.float_toolbox('float4')
    ...
    if sim == 1:
        ...
        float_data = semfl.float_dataset_gen(
            Nb, floatx, mod, 0, 0, p_s=p_s, huffman=huffman)
        perf_meas = mf.performance_measures(
            Nerr_min=Nerr_min, it_max=it_max, sel_crit='ber')  # 'it'
        ...
        for ii, snr in enumerate(sim_par.SNR):
            if snr not in perf_meas.SNR:
                while perf_meas.stop_crit():    # Simulate until 1000 errors or stop after it_max iterations
                    ...
                    z = np.concatenate(
                        (1 - b_huffseq[..., np.newaxis], b_huffseq[..., np.newaxis]), axis=-1)
                    ...
                    if huffman == 0:
                        # p_ba = mod.alpha
                        p_x = lin_det_soft(x_est, Phi_ee, mod.m, p_ba)
                    else:
                        p_x = lin_det_soft(x_est, Phi_ee, mod.m, np.array(
                            [0.5, 0.5])[np.newaxis, :])  # p_ba = mod.alpha
                    _, p_b0 = com.symprob2llr(p_x, mod.M)
                    p_b = np.concatenate((p_b0, 1 - p_b0), axis=-1)
                    b_r = (p_b[..., 0] < 0.5) * 1

                    # Huffman decoding
                    b_huffdec = mop.int2bin(np.array(huffman.decoding(
                        (p_b[..., 0] < 0.5) * 1)), N=floatx.N_bits)
                    str2 = ''.join(str(e) for e in (b_huffdec.flatten() * 1))
                    str1 = ''.join(str(e)
                                   for e in floatx.float2bit(s).flatten())
