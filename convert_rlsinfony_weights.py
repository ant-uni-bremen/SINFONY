        ...
        for i in range(num_residuals):
            if bottleneck == True:
                if i == 0:
                    ...
                    self.residual_layers.append(Residual_bottleneck(
                        num_channels, kernel_initializer=kernel_initializer, kernel_regularizer=kernel_regularizer, preactivation=preactivation))
            else:
...
def resnet(shape=(32, 32, 3), classes=10, filters=64, num_res=2, num_resblocks=4, preactivation=True, bottleneck=False):
    ...
    x = tf.keras.layers.Conv2D(filters, kernel_size=7, strides=2, padding='same',
                               kernel_initializer=weight_init, kernel_regularizer=weight_decay)(x_input)
    ...
    for ind in range(0, num_resblocks):
        if ind == 0:
            x = ResnetBlock(2 ** ind * filters, num_res, first_block=True, kernel_initializer=weight_init,
                            # first_block = True
                            kernel_regularizer=weight_decay, preactivation=preactivation, bottleneck=bottleneck)(x)
        else:
            x = ResnetBlock(2 ** ind * filters, num_res, kernel_initializer=weight_init,
                            kernel_regularizer=weight_decay, preactivation=preactivation, bottleneck=bottleneck)(x)
    # Step 3 (Final Layers)
...
def resnet_cifar(shape=(32, 32, 3), classes=10, filters=16, num_res=3, num_resblocks=3, preactivation=True, bottleneck=False):
    ...
    model = tf.keras.models.Model(
        inputs=x_input, outputs=x, name='ResNet_CIFAR10')
    return model
    ...
    if n_rx > -1:
        ...
        if rx_list == False and rx_same == 1:
            ...
            for indl in range(0, num_layer):
                ...
                dec_layerlist.append(Dense(image_fac ** 2 * n_rx, activation="relu",
...
def resnet_cifar_ae(shape=(32, 32, 3), classes=10, filters=16, num_res=2, num_resblocks=3, preactivation=True, bottleneck=False, axnorm=0, n_tx=-1, n_rx=-1, rx_same=1, rx_linear=False, num_layer=1, sigma=np.array([0, 0]), image_fac=2):
    ...
    # Tx
    if image_fac >= 2:
        tx = resnet_cifar_multitx(shape=shape, filters=filters, num_res=num_res, num_resblocks=num_resblocks,
        ...
        tx = resnet_cifar_tx(shape=shape, filters=filters, num_res=num_res, num_resblocks=num_resblocks,
                             preactivation=preactivation, bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, num_layer=num_layer)
        label = 'ResNet_CIFAR10_AE'

    # Rx
    rx = resnet_cifar_rx(shape=tx.layers[-1].output_shape[1:], classes=classes, n_rx=n_rx,
                         num_layer=num_layer, rx_same=rx_same, rx_linear=rx_linear, image_fac=image_fac)
...
# Function that implements one transmitter training iteration using RL.
    ...
    def train_tx(opt_tx, trainX, trainY, sigma, perturbation_var=tf.constant(0.0, tf.float32)):
        ...
        with tf.GradientTape() as tape:
            ...
            _, tx_loss, rx_loss, acc = model(
                trainX, trainY, sigma, perturbation_var())
    ...
    def train_rx(opt_rx, trainX, trainY, sigma):
        ...
        with tf.GradientTape() as tape:
            ...
            _, _, rx_loss, acc = model(trainX, trainY, sigma)
        # Computing and applying gradients
    ...
    for i in range(epochs):
        ...
        for batchX, batchY in get_batch(trainX, trainY, training_batch_size):
            if ii % total_steps >= rx_steps:
                ...
                rx_loss, acc, tx_loss = train_tx(
                    optimizer_tx, batchX, batchY, sigma, perturbation_var=perturbation_var)
                if (valX is None) or (valY is None):
                    print_str = '[Tx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}, Acc: {:.2f}, PG: {:.4f}'.format(
                        i + 1, epochs, ii + 1, Ne, rx_loss.numpy(), acc.numpy(), tx_loss.numpy())
                else:
                    _, tx_val_loss, rx_val_loss, acc_val = model(
                        valX, valY, sigma, perturbation_var())
                    print_str = '[Tx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}/{:.4f}, Acc: {:.2f}/{:.2f}, PG: {:.4f}/{:.4f}'.format(
                        i + 1, epochs, ii + 1, Ne, rx_loss.numpy(), rx_val_loss.numpy(), acc.numpy(), acc_val.numpy(), tx_loss.numpy(), tx_val_loss.numpy())
                    perf_meas['rx_val_loss'].append(rx_val_loss.numpy())
                    perf_meas['acc_val'].append(acc_val.numpy())
                    perf_meas['tx_val_loss'].append(tx_val_loss.numpy())
                perf_meas['tx_loss'].append(tx_loss.numpy())
                ...
                if (valX is None) or (valY is None):
                    print_str = '[Rx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}, Acc: {:.2f}'.format(
                        i + 1, epochs, ii + 1, Ne, rx_loss.numpy(), acc.numpy())
                else:
                    _, _, rx_val_loss, acc_val = model(valX, valY, sigma)
                    print_str = '[Rx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}/{:.4f}, Acc: {:.2f}/{:.2f}'.format(
                        i + 1, epochs, ii + 1, Ne, rx_loss.numpy(), rx_val_loss.numpy(), acc.numpy(), acc_val.numpy())
                    perf_meas['rx_val_loss'].append(rx_val_loss.numpy())
                    perf_meas['acc_val'].append(acc_val.numpy())
            perf_meas['rx_loss'].append(rx_loss.numpy())
            perf_meas['acc'].append(acc.numpy())
            ...
            if ii % it_print == 0:
                print(print_str + ', Time: {:04.2f}s, Tot. time: '.format(
                    time.time() - start_time) + print_time(time.time() - start_time0))  # , end = '\r')
                start_time = time.time()
            ii = ii + 1
...
# Once alternating training is done, the receiver is fine-tuned.
    ...
    for i in range(epochs_fine):
        ...
        for batchX, batchY in get_batch(trainX, trainY, training_batch_size):
            ...
            if (valX is None) or (valY is None):
                print_str = '[Rx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}, Acc: {:.2f}'.format(
                    i + 1, epochs_fine, ii + 1, Ne, rx_loss.numpy(), acc.numpy())
            else:
                _, _, rx_val_loss, acc_val = model(valX, valY, sigma)
                print_str = '[Rx] Epoch: {}/{}, Batch: {}/{}, CE: {:.4f}/{:.4f}, Acc: {:.2f}/{:.2f}'.format(
                    i + 1, epochs_fine, ii + 1, Ne, rx_loss.numpy(), rx_val_loss.numpy(), acc.numpy(), acc_val.numpy())
...
def load_dataset(dataset='mnist'):
    ...
    if dataset == 'cifar10':
        ...
        import pandas as pd
        X = pd.read_csv(data_file, sep="\s", header=None,
                        engine='python')  # X.loc[:, 0]
        X2 = pd.read_csv(data_file2, sep="\s", header=None, engine='python')
        # Add index of alphanumerically ordered data set to X2
        indr = np.argmax(X.sort_values(by=0, ascending=True).loc[:, 0].to_numpy(
        ) == X2.loc[:, 0].to_numpy()[:, None], axis=-1)
        X2[3] = indr.tolist()
        labels_list = X.sort_values(by=0, ascending=True).loc[:, 1].to_numpy()
        class_names = pd.read_csv(
            data_dir + 'landmarks_map-proj-v3_2_classmap.csv', sep=',', header=None, engine='python')
...
if __name__ == '__main__':
    ...
    # Simulation
    load = True										# Load model and reevaluate: False (default)
    ...
    model_dir = os.path.join(path0, path)
    weight_path = os.path.join(path0, pathfile + '_weights.h5')
    ...
    # None, testx[:100, ...], testx[:1000, ...]
    valX = testx[:100, ...]
    # None, testy[:100, ...], testy[:1000, ...]
    valY = testy[:100, ...]
    # RL training
    # (0) default AE, (1) Reinforcement learning training, (2) AE trained with rl-based training implementation
    rl = 1
    # Print after 1 (default) iterations training progress
    it_print = 10
    ...
    exp_boundaries = list(
        np.round(np.array(per_epoch_bound) / 2 * iter_epoch).astype('int'))
    expl_var = pertubation_variance_schedule(
        expl_values, exp_boundaries)
    ...
    # 0: individual rx, 1: same rx (default), 2: one joint rx
    rx_same = 1
    rx_linear = False			# False: No final layer for each Rx module
    # Division of picture by 2 (default) to create 4 patches
    image_fac = 2
    ...
    if optimizer.lower() == 'sgdlrs':
        ...
        boundaries = list(np.round(np.array(epoch_bound)
                                   * iter_epoch).astype('int'))
        # [0.1, 0.01, 0.001] for ae training / [0.001, 0.0001, 0.00001] for adam / [1e-3, 1e-4, 1e-5] for rl training / [1e-3, 1e-4] for rl CIFAR training sgdlr2
    ...
    # 16 for CIFAR10 (adjust for MNIST with only 1 channel instead of 3?)
    filters = int(dataset_shape[1] / 2)
    ...
    resnet_lnum = 2 * num_resblocks * num_res + 2
        ...
        if fname.endswith(model_ext):
            if not fname.endswith('.npz') and not fname.endswith('.h5') and not fname.endswith('.yaml'):
                ...
                try:
                    if load == False:
                        ...
                        if ae == True:
                            ...
                            if noise == False:
                                ...
                                # training with noise
                                sigma_train = mop.csigma(
                                    np.array([snr_min_train, snr_max_train]))[::-1]
                            # Select SINFONY or RL-SINFONY
                            if rl == 1:
                                # RL-SINFONY
                                model = resnet_cifar_rl(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks, preactivation=preactivation,
                                                        bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, n_rx=n_rx, rx_same=rx_same, rx_linear=rx_linear, num_layer=num_layer, image_fac=image_fac)
                            elif rl == 2:
                                # SINFONY trained via RL-SINFONY training loop
                                model = resnet_cifar_ae2(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks, preactivation=preactivation,
                                                         bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, n_rx=n_rx, rx_same=rx_same, rx_linear=rx_linear, num_layer=num_layer, image_fac=image_fac)
                            else:
                                # SINFONY
                                model, tx, rx = resnet_cifar_ae(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks, preactivation=preactivation,
                            ...
                            model = resnet_cifar(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res,
                                                 num_resblocks=num_resblocks, preactivation=preactivation, bottleneck=bottleneck)
                    else:
                        # Load existing model:
                        print('Loading model...')
                        if rl >= 1 and ae == True:
                            ...
                            if rl == 1:
                                model = resnet_cifar_rl(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks,
                                                        preactivation=preactivation, bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, n_rx=n_rx, rx_same=rx_same, num_layer=num_layer, image_fac=image_fac)
                                model2 = resnet_cifar_rl(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks,
                                                         preactivation=preactivation, bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, n_rx=n_rx, rx_same=rx_same, num_layer=num_layer, image_fac=image_fac)
                            elif rl == 2:
                                model = resnet_cifar_ae2(shape=dataset_shape[1:], classes=classes, filters=filters, num_res=num_res, num_resblocks=num_resblocks,
                                                         preactivation=preactivation, bottleneck=bottleneck, axnorm=axnorm, n_tx=n_tx, n_rx=n_rx, rx_same=rx_same, num_layer=num_layer, image_fac=image_fac)
                            model.load_weights(os.path.join(
                                model_path, fname), skip_mismatch=False)
                        else:
                            # Load SINFONY model
                    ...
                    model = model_fix(model)
                    if save_weights is True:
                        model.save_weights(weight_path)
                        print("✅ Save successful.\n")
                    else:
                        model.load_weights(
                            weight_path, skip_mismatch=False, by_name=True)
                        print("✅ Load successful.\n")
                    print('Model weights saved.')

                    # print('Test loading model...')
                    # model2(testnorm, testy, np.array(
                    ...
                    #         results = rl_based_training(model, train_norm, trainy, opt, opt_tx, opt_rx2, valX=valX, valY=valY, epochs=Ne, epochs_fine=Ne_fine,
                    #                                     tx_steps=tx_steps, rx_steps=rx_steps, training_batch_size=bs, sigma=sigma_train, perturbation_var=expl_var, it_print=it_print)
                    #         # Save model weights:
                    #         print('Saving model weights...')
                    ...
                    #         history = model.fit(train_norm, trainy, epochs=Ne, batch_size=bs, validation_data=(
                    #             testnorm, testy), callbacks=[batch_tracking, model_checkp, early_stopping], verbose=verbose)
                    ...
                    #                     _, _, lossii, accii = model(
                    #                         testnorm, testy, sigma=tf.constant(sigma_test, dtype='float32'))
                    #                 else:
                    ...
                    #                 print('Validation Round: {}/{}, CE: {:.4f}, Acc: {:.2f}'.format(ii2 + 1,
                    #                       val_rounds, lossi, acci) + ', Time: ' + print_time(time.time() - start_time2))
                    #             # Append list with evaluation for each SNR value
                    #             eval_meas[0].append(lossi)
                    #             eval_meas[1].append(acci)
                    #             print('Iteration: {}/{}, SNR: {}, CE: {:.4f}, Acc: {:.2f}'.format(ii + 1, len(
                    #                 SNR), snr, lossi, acci) + ', Time: ' + print_time(time.time() - start_time))
                    ...
                    #         if rl >= 1:
                    #             _, _, lossi, acci = model(
                    #                 testnorm, testy, sigma=tf.constant([0, 0], dtype='float32'))
                    #         else:
