...
# Include parent folders, where own packages are
sys.path.append('..')                       # NOQA
sys.path.append('../..')                    # NOQA
...
# Own packages
from my_equalizer import lin_det_soft
...
class float_dataset_gen():
    ...
    def __init__(self, Nb, floatx, modulation, snr_min, snr_max, p_s, huffman=0):
        '''Input -----------------------------------------
        Nb: Batch size
        floatx: Floating point object
        modulation: Modulation object
        snr_min: Minimum SNR
        snr_max: Maximum SNR
    ...
    def gen_data(self, Nb=None):
        ...
        if Nb == None:
            Nb = self.N_batch
        [y, sigma, s, b] = float_model(
            Nb, self.p_s, self.floatx, self.modulation, self.snr_min, self.snr_max, self.huffman)
        self.s = s
        self.b = b
        self.sigma = sigma
        self.y = y
...
def float_source(Nb, p_s, floatx):
    ...
    if floatx.float_name == 'float32' or floatx.float_name == 'float64':
        ...
        if p_s[0] == 0:
            ...
            _, s = floatx.float2bitint(
                np.random.normal(p_s[1], np.sqrt(p_s[2]), (Nb)))
        elif p_s[0] == 1:
            ...
            bint = np.random.randint(
                0, 2 ** (floatx.N_bits - 1) - 2 ** floatx.N_sig, Nb)
            indrand = np.random.randint(0, 1 + 1, Nb)
            bint[indrand == 1] = np.random.randint(
                2 ** (floatx.N_bits - 1), 2 ** floatx.N_bits - 2 ** floatx.N_sig, bint[indrand == 1].shape)
...
def semantic_channel(b, modulation, snr_min, snr_max):
    ...
    sigma: Standard deviations of AWGN channel
        '''
    # Modulation
    x = modulation.modulate(b[..., np.newaxis], axis=-1)
    # Channel
    sigma = mop.csigma(np.random.uniform(
        snr_min, snr_max, x.shape[0]))[..., np.newaxis]
    y = com.awgn(x, np.repeat(sigma, x.shape[-1], axis=-1), compl=0)
    return y, sigma


def float_model(Nb, p_s, floatx, modulation, snr_min, snr_max, huffman=0):
    ...
    y, sigma = semantic_channel(b, modulation, snr_min, snr_max)
    return y, sigma, s, b
...
def compute_prior(floatx, mode=0, fmode=True):
    ...
    if fmode == True and (floatx.float_name == 'float32' or floatx.float_name == 'float64'):
        ...
        if mode == 0:
            ...
            # Uniform discrete distribution
            p_s = np.array([mode, 2 ** (floatx.N_bits - 1) - 2 **
                           floatx.N_sig, 2 ** floatx.N_bits - 2 ** floatx.N_sig])
        ...
        if mode == 0:
            ...
            p_s = mfl.sequence_prior_data(floatx, load_data_explseismic(
                floatx.N_bits, mode=1))  # choose dataset/path with mode
        elif mode == 3:
            # For convenience: Both data sets from DLR merged
            p_s = mfl.sequence_prior_data(
                floatx, load_data_explseismic(floatx.N_bits, mode=0))
        elif mode == 4:
...
def load_data_semcom(flatten=True, train=True):
    ...
    path = os.path.join('models', dataset)
    ...
    print('Model loaded.')
    import datasets
    trainx, _, testx, _ = datasets.load_dataset(dataset)
    train_norm, test_norm = datasets.prep_pixels(trainx, testx)
    if train is True:
        # .numpy() # for total test set
        data = model.layers[1].predict(train_norm)
    else:
...
def integral_rec(s, f_s):
    ...
    # Reordering
    indord2 = np.argmax(np.arange(0, s_ord.shape[0])[
                        :, np.newaxis] == indord[np.newaxis, :], axis=-1)
...
def sequence_prior_sampled_gaussian(floatx, N_s=10 ** 9, s_mean=0, s_var=1, step=100000):
    ...
    for _ in range(0, int(N_s / step)):
        bint, _ = floatx.float2bitint(
            np.random.normal(s_mean, np.sqrt(s_var), (step)))
...
def sequence_prior_sampled_uniform(floatx, N_s=10 ** 9, step=100000):
    ...
    '''
    p_s = np.zeros(2 ** floatx.N_bits)
    s_max = (1 - 2 ** (- floatx.N_sig - 1)) * \
        2 ** (2 ** floatx.N_exp - 1 - floatx.expbias)
    for _ in range(0, int(N_s / step)):
...
def sequence_est(p_s, y, sigma, modulation, floatx, step=100, mode=0, tikzplt=0, it_print=0):
    ...
    x = modulation.modulate(b_poss2[..., np.newaxis], axis=-1)
    ...
    for indj in range(0, int(y.shape[0] / step)):
        ...
        norm2 = np.sum(
            (yj[:, np.newaxis, :] - x.reshape((1, x.shape[0], x.shape[1]))) ** 2, axis=-1)
        ...
        if mode == 0 or mode == 2:
            ...
            s_mean[indj * step:(indj + 1) * step] = np.sum(p_sr *
                                                           s_poss[np.newaxis, :], axis=-1)
            b_r[indj * step:(indj + 1) * step,
                :] = floatx.float2bit(s_mean[indj * step:(indj + 1) * step])
            s_var[indj * step:(indj + 1) * step] = np.sum(p_sr * (s_poss[np.newaxis, :] -
                                                                  s_mean[indj * step:(indj + 1) * step, np.newaxis]) ** 2, axis=-1)
            # -------- Plot the posterior -------------------------------------------------------
            if tikzplt == 1 and indj == 0:
                indx = 0
                plot_posterior(s_poss, p_sr[indx, :],
                               s_mean[indx], s_var[indx], x_lim=4)
...
def plot_posterior(s_poss_fin, p_sr, s_mean, s_var, x_lim=4):
    ...
    s_int_reord = integral_rec(
        s_poss_fin, np.ones(s_poss_fin.shape, dtype=np.int8))
    ...
    plt.figure(1)
    plt.stem(s_poss_fin, p_sr / np.max(p_sr), 'b-x', markerfmt='bx',
             basefmt=" ", label=r'$p(\tilde{s}|\mathbf{y})$')
    plt.plot(s_poss_ord, p_srcon / np.max(p_srcon),
             'r-o', label=r'$p(s|\mathbf{y})$ step-wise')
...
def ml_receiver_bits(m, n):
    ...
    layer1 = Dense(m, activation='relu',
                   kernel_initializer='he_uniform')(inputs)
    layer2 = Dense(m, activation='relu',
                   kernel_initializer='he_uniform')(layer1)
...
def ml_receiver_reg_heteroscedastic(m, n):
    ...
    layer2 = Dense(m, activation='relu',
                   kernel_initializer='he_uniform')(layer1)
    # mu NN
    mu_layer1 = Dense(m, activation='relu',
                      kernel_initializer='he_uniform')(layer2)
    mu_layer2 = Dense(m, activation='relu',
                      kernel_initializer='he_uniform')(mu_layer1)
    mu_out = Dense(1, activation='linear')(mu_layer2)
    # sigma NN
    sigma_layer1 = Dense(m, activation='relu',
                         kernel_initializer='he_uniform')(layer2)
    sigma_layer2 = Dense(m, activation='relu',
                         kernel_initializer='he_uniform')(sigma_layer1)
    ...
    # Add loss to model
    rx.add_loss(loss)
    rx.add_metric(heteroscedastic_mse_metric(labels, mu_out),
                  name='mseh', aggregation='mean')
    return rx
...
def ml_transmitter(m, n, input_shape=1, axnorm=0):
    ...
    tx_layer1 = Dense(m, activation='relu', kernel_initializer='he_uniform')(
        tx_in)  # , kernel_initializer='he_uniform' # for RELU
    tx_layer2 = Dense(m, activation='relu',
                      kernel_initializer='he_uniform')(tx_layer1)
...
def mse_heteroscedastic(y_true, mu, sigma):
    ...
    mse_heteroscedastic = tf.keras.backend.mean(tf.keras.backend.square(
        (y_true - mu) / sigma2) / 2 + tf.keras.backend.log(sigma2) + 0.5 * tf.keras.backend.log(2 * np.pi), axis=0)
    return mse_heteroscedastic


def heteroscedastic_mse_metric(true_outputs, predicted_output):
    '''Mean Squared Error (MSE) metric for heteroscedastic training
    INPUTS
    true_outputs: True outputs
    predicted_output: Predicted output
    OUTPUT
    mse: Mean Squared Error loss computed manually
    '''
    mse = tf.keras.backend.mean(tf.keras.backend.square(
        true_outputs - predicted_output), axis=0)
    return mse
...
if __name__ == '__main__':
    ...
    # 0: Gaussian, 1: Uniform (continuous), 2: Data-based DLR (gradients), 3: Data-based DLR (gradients + models), 4: Data-based SINFONY, 5: Uniform (per class)
    prior = 3
    it_max = 100            						# Maximum number of iterations per SNR value: 100
    Nb = 100000             						# Evalution batch size: 10000, 100000
    # Sequence estimator steps: [home: 100, work: 1000]
    seq_step = 1000
    step_size = 1									# EbN0 step size
    EbN0_range = [-6, 14]   						# Validation Eb/N0: [-6, 14]
    R_c = 1											# Provide code rate for saves
    # Floating point object: float4/8/16
    floatx = mfl.float_toolbox('float16')
    print(f"# of bits: {Nb * floatx.N_bits * it_max}")
    print(f"# of floats: {Nb * it_max}")
    modulation = com.modulation('BPSK')					# Modulation object: BPSK, ...
    # Set correct alpha dimensions
    modulation.alpha = 1 / modulation.M * \
        np.ones((int(floatx.N_bits / np.log2(modulation.M)), modulation.M))
    sim_par = mf.simulation_parameters(
        1, 1, 1, modulation, 1, EbN0_range, rho=0)		# Simulation parameters object
    sim_par.snr_gridcalc(step_size)
    p_ba, p_s = compute_prior(floatx, mode=prior)

    # Path and save
    path = os.path.join('curves_float', modulation.mod_name,
                        floatx.float_name, 'prior' + str(prior))
    filename = 'RES_' + algo + fn_ext
    pathfile = os.path.join(path, filename)
    saveobj = mf.savemodule(form='npz')

    perf_meas = mf.performance_measures(
        Nerr_min=100, it_max=it_max, sel_crit='it')
    if load_set == 1:
    ...
    float_data = float_dataset_gen(
        bs, floatx, modulation, snr_min_train, snr_max_train, p_s)
    ...
    if algo in ('DNN', 'DNNh'):
        ...
        if algo == 'DNN':
            ...
            # metrics=['mean_squared_error']) # 'categorical_crossentropy', 'binary_crossentropy', 'mse'
            model.compile(optimizer=opt, loss='mean_squared_error')
            val_data = (
                dnn_input(data_val[0], data_val[1], mode=llr), data_val[2])
            hmode = 0
        elif algo == 'DNNh':
            # Learn posterior floating point value distribution with heteroscedastic model
            print('Start training Rx heteroscedastic DNN...')
            model = ml_receiver_reg_heteroscedastic(
                2 * floatx.N_bits, floatx.N_bits)
            model.compile(optimizer=opt)
            val_data = [dnn_input(data_val[0], data_val[1],
                                  mode=llr), data_val[2]]
            hmode = 1
        else:
    ...
    if algo in ('AE', 'AEh', 'AEb', 'AEbh'):
        ...
        if algo in ('AE', 'AEb'):
            ...
            model, _, _ = ml_com_reg(NL, Ntx, mop.csigma(np.array([snr_min_train, snr_max_train]))[
                                     ::-1], input_shape=input_shape, axnorm=axnorm)
            model.compile(optimizer=opt, loss='mean_squared_error')
            hmode = 0
        elif algo in ('AEh', 'AEbh'):
            print('Start training heteroscedastic AE...')
            model, _, _ = ml_com_reg(NL, Ntx, mop.csigma(np.array([snr_min_train, snr_max_train]))[
                                     ::-1], input_shape=input_shape, axnorm=axnorm, hmode=1)
            model.compile(optimizer=opt)
    ...
    if algo == 'hardbitNN':
        ...
        history2 = model2.fit(data_generator(float_data, outbin=1, llr=llr),
                              ...
                              batch_size=None,
                              validation_data=(
                                  dnn_input(data_val[0], data_val[1], mode=llr), data_val[3]),
                              validation_steps=1,
                              verbose=0,  # 2
                              )
        print('Tot. time: ' + mf.print_time(time.time() - start_time))
    ...
    for ii, snr in enumerate(sim_par.SNR):
        if snr not in perf_meas.SNR:
            while perf_meas.stop_crit():    # Simulate until 1000 errors or stop after it_max iterations
                ...
                z = np.concatenate(
                    (1 - b[..., np.newaxis], b[..., np.newaxis]), axis=-1)

                # Stochastically independent prior Equalizer
                if algo == 'hardbit':
                    x_est = y
                    Phi_ee = mop.tvec2diag(sigma ** 2)
                    p_x = lin_det_soft(x_est, Phi_ee, modulation.m,
                                       p_ba)  # p_ba = modulation.alpha
                    _, p_b0 = com.symprob2llr(p_x, modulation.M)
                    p_b = np.concatenate((p_b0, 1 - p_b0), axis=-1)

                # NN equalizer
                if algo == 'hardbitNN':
                    p_b1 = model2.predict(
                        dnn_input(y, sigma, mode=llr))[..., np.newaxis]
                    p_b = np.concatenate((1 - p_b1, p_b1), axis=-1)
                    b_r = (p_b[..., 0] < 0.5) * 1

                # 1. Hard bit decision estimator, Single-bit detector
                if algo in ('hardbit', 'hardbitNN'):
                    b_r = (p_b[..., 0] < 0.5) * 1
                ...
                if algo in ('DNN', 'DNNh'):
                    if algo == 'DNN':
                        ...
                        s_r, s_r_var = model.predict(
                            [dnn_input(y, sigma, mode=llr), np.zeros(s.shape)])  # dummy
                        s_r_var = s_r_var[:, 0]
                    s_r = s_r[:, 0]
                    # _, s_r = floatx.float2bitint(s_r)       # Discretization from float64 to floatx
                    b_r = floatx.float2bit(s_r)
                    p_b = np.concatenate(
                        (1 - b_r[:, :, np.newaxis], b_r[:, :, np.newaxis]), axis=-1)

                # 3. AE (heteroscedastic)
                if algo in ('AE', 'AEh', 'AEb', 'AEbh'):
                    model.layers[2].set_weights(
                        [np.array([mop.csigma(snr), mop.csigma(snr)])])
                    if algo in ('AEb', 'AEbh'):
                        input0 = b
                    else:
                        input0 = s
                    if algo in ('AE', 'AEb'):
                        ...
                        s_r, s_r_var = model.predict(
                            [input0, np.zeros(input0.shape)])  # dummy
                        s_r_var = s_r_var[:, 0]
                    s_r = s_r[:, 0]
                ...
                if algo == 'MAPseq':
                    s_r, b_r, _, _ = sequence_est(
                        p_s, y, sigma, modulation, floatx, step=seq_step, mode=1)  # home: 100, work: 1000
                    p_b = np.concatenate(
                        (1 - b_r[:, :, np.newaxis], b_r[:, :, np.newaxis]), axis=-1)

                # 5. Mean Estimator (Moment Matching)
                if algo == 'Meanseq':
                    _, b_r, s_r, _ = sequence_est(
                        p_s, y, sigma, modulation, floatx, step=seq_step, mode=2)  # home: 100, work: 1000
                    p_b = np.concatenate(
                        (1 - b_r[:, :, np.newaxis], b_r[:, :, np.newaxis]), axis=-1)
                    # Discretization from float64 to floatx
                    _, s_r = floatx.float2bitint(s_r)
                ...
                if algo == 'analog':
                    isfin = np.isfinite(floatx.x_poss)
                    sigma_s = np.sqrt(
                        np.sum(floatx.x_poss[isfin].astype('float64') ** 2 * p_s[isfin]))
                    ...
                    y = com.awgn(
                        s, sigma_s * sigma[:, 0] / np.sqrt(floatx.N_bits))
                    s_r = y
                    # Discretization from float64 to floatx
                ...
                perf_meas.eval(z, p_b, modulation)
            ...
            [print_str, sv_flag] = perf_meas.err_saveit(
                sim_par.SNR[ii], sim_par.EbN0[ii], sim_par.EbN0[ii] - 10 * np.log10(R_c))
            print(f"{len(sim_par.SNR) - ii}, {print_str}")
            if sv_flag:
                # Save results to file
                saveobj.save(pathfile, perf_meas.results())
