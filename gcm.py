...
class AttentionWeightLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs=None):
        ...
        positive_weights = tf.keras.activations.relu(self.attention_weights)
...
@tf.keras.utils.register_keras_serializable()
class WeightedDistanceLayer(tf.keras.layers.Layer):
...
class SimilarityLayer(tf.keras.layers.Layer):
    def __init__(self, similarity_gradient=1.0, **kwargs):
        ...
        self.c = tf.Variable(
            initial_value=similarity_gradient,
            trainable=True,
    ...
    def call(self, inputs):
        ...
        log_similarities = -tf.keras.activations.relu(self.c) * distances
...
class ExemplarMemoryLayer(tf.keras.layers.Layer):
...
class GeneralizedContextModel(tf.keras.Model):
...
class GeneralizedContextModelDifferentiableMemory(tf.keras.Model):
    '''Generalized Context Model implemented in tensorflow
    '''
    ...
    def __init__(self, exemplars_dim, labels, similarity_gradient=1.0):
        ...
        self.labels = tf.Variable(
            # shape: (N,)
            labels, dtype=tf.int32, trainable=False, name='exemplar_labels')
...
class GeneralizedContextModelOld(tf.keras.Model):
    '''Generalized Context Model implemented in tensorflow
    '''

    def __init__(self, exemplars, labels, similarity_gradient=1.0):
        super().__init__()
        self.exemplars = tf.Variable(
            # shape: (N, D)
            exemplars, dtype=tf.float32, trainable=False, name='exemplars_memory')
        self.labels = tf.Variable(
            # shape: (N,)
            labels, dtype=tf.int32, trainable=False, name='exemplar_labels')
        # Learnable scalar
...
def new_optimizer(opt_class=tf.keras.optimizers.SGD, opt_config={"learning_rate": 0.01, "momentum": 0.9}):
...
class OutputSelector(tf.keras.layers.Layer):
...
def wrap_with_output_masking(base_model, mask):
    ...
    Args:
        base_model (tf.keras.Model): the model whose output will be wrapped
        mask: masks the output tensor

    Returns:
        tf.keras.Model: a new model with same input as base_model, but masked output
    """
    ...
    return tf.keras.Model(inputs=input_tensor, outputs=masked_output, name=base_model.name + "_masked")
...
if __name__ == '__main__':
    ...
    # Possible data sets: mnist, cifar10, fraeser (human rover), fraeser64,
    # hise, hise64, hise256, speechcommands, urbansound8k
    # Number after dataset name is the resolution of the images
    wrapper = 'cifar10'
    simulation = 'working_memory'          # snr, memory, working_memory

    load = False
    filename_gcm = 'GCM_Ne1_Na20_val10'
    ...
    if gcm_input != 2:
        template_files, dataset_name = sw.template_models(wrapper)
    ...
    if not learning_rate_schedule2:
        ...
        learning_rate_schedule2 = tf.keras.optimizers.schedules.PiecewiseConstantDecay(
            boundaries2, values2)
    ...
    if simulation == 'memory':
        ...
        if joint_training is False:
            if load is True:
                ...
                print('Loading model...')
                gcm = tf.keras.models.load_model(pathfile)
                print('Model loaded.')
            else:
                # Save the GCM model
                print('Saving model...')
        ...
        if gcm_input != 2:
            if load is True and joint_training is True:
                # Load existing model:
                print('Loading model...')
                sinfony_gcm = tf.keras.models.load_model(pathfile)
                print('Model loaded.')
            else:
